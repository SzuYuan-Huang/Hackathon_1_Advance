...
namespace Hackathon_5_Advance
{
    ...
    {
        Dictionary<int, string> myConstellation;
        Dictionary<string, Constellation> data;
        ...
        {
            int sum = 0;
            string yourBirthDay = dateTimePicker1.Value.ToShortDateString();
            string[] cut = yourBirthDay.Split('/');
            for(int i=0;i<cut.Length;i++)
            {
                char[] firstTemp = cut[i].ToCharArray();
                for(int j=0;j<firstTemp.Length;j++)
                {
                    sum += int.Parse(firstTemp[j].ToString());
                }
            }

            while(sum > 9)
            {
                char[] secondTemp = sum.ToString().ToCharArray();
                sum = 0;
                for (int i = 0; i < secondTemp.Length; i++)
                {
                    sum += int.Parse(secondTemp[i].ToString());
                }
            }

           /* string constellation = GetAtomFromBirthday(dateTimePicker1.Value);*/

            data = findConstellation();
            int yourYear = int.Parse(cut[0]);
            int yourMonth = int.Parse(cut[1]);
            int yourDay = int.Parse(cut[2]);

            DateTime yourDt = new DateTime(yourYear,yourMonth,yourDay);

            foreach(var dt in data)
            {
                if(dt.Value.startDate <= yourDt && yourDt <= dt.Value.endDate)
                {
                    constellation = dt.Key.ToString();
                }
            }
            ...
            {
                case "牡羊座":
                    myConstellation = CreateAries();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                case "金牛座":
                    myConstellation = CreateTaurus();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                case "雙子座":
                    myConstellation = CreateGemini();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                case "巨蟹座":
                    myConstellation = CreateCancer();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                case "獅子座":
                    myConstellation = CreateLeo();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                case "處女座":
                    myConstellation = CreateVirgo();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                case "天秤座":
                    myConstellation = CreateLibra();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                case "天蠍座":
                    myConstellation = CreateScorpio();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                case "射手座":
                    myConstellation = CreateSagittarius();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                case "摩羯座":
                    myConstellation = CreateCapricorn();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                case "水瓶座":
                    myConstellation = CreateAquarius();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                case "雙魚座":
                    myConstellation = CreatePieces();
                    label2.Text = "你的星座是" + constellation + Environment.NewLine + "你的生命靈數" + sum + Environment.NewLine + myConstellation[sum];
                    break;
                default:
                    break;
        ...
        private Dictionary<string,Constellation> findConstellation()
        {
            var result = new Dictionary<string, Constellation>();
            result.Add("白羊座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 3, 21), endDate = new DateTime(dateTimePicker1.Value.Year, 4, 19) });
            result.Add("金牛座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 4, 20), endDate = new DateTime(dateTimePicker1.Value.Year, 5, 20) });
            result.Add("雙子座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 5, 21), endDate = new DateTime(dateTimePicker1.Value.Year, 6, 21) });
            result.Add("巨蠍座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 6, 22), endDate = new DateTime(dateTimePicker1.Value.Year, 7, 22) });
            result.Add("獅子座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 7, 23), endDate = new DateTime(dateTimePicker1.Value.Year, 8, 22) });
            result.Add("處女座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 8, 23), endDate = new DateTime(dateTimePicker1.Value.Year, 9, 22) });
            result.Add("天秤座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 9, 23), endDate = new DateTime(dateTimePicker1.Value.Year, 10, 23) });
            result.Add("天蠍座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 10, 24), endDate = new DateTime(dateTimePicker1.Value.Year, 11, 21) });
            result.Add("射手座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 11, 22), endDate = new DateTime(dateTimePicker1.Value.Year, 12, 20) });
            result.Add("魔蠍座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 12, 21), endDate = new DateTime(dateTimePicker1.Value.Year, 1, 20) });
            result.Add("水瓶座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 1, 21), endDate = new DateTime(dateTimePicker1.Value.Year, 2, 19) });
            result.Add("雙魚座", new Constellation { startDate = new DateTime(dateTimePicker1.Value.Year, 2, 20), endDate = new DateTime(dateTimePicker1.Value.Year, 3, 20) });
            return result;
        }
